import { stringToU8, u16ToU8 } from "./util.js";
...
export const validate = {
    ...
    biome: v => {
        v = +v;
        if (isNaN(v) || v < 0 || v > 6) {
            throw new Error("biome must be a number between 0 and 6");
        }
    ...
    directConnect: v => {
        ...
        if (split.length !== 3) {
            throw new Error("directConnect must be a string in the format 'address,modtd,timeZone'");
        }

        const [address, modtd, timeZone] = split;
        ...
        if (typeof modtd !== "string" || modtd.length < 1 || modtd.length > 128) {
            throw new Error("modtd must be a string between 1 and 128 characters long");
        }
...
export default class Lobby {
    ...
    constructor(ownerSocket, gameName) {
        ...
        /** @type {{address:string,modtd:string,timeZone:number}|null} */
    ...
    setDirectConnect(address, modtd, timeZone) {
        if (this.isPrivate) {
    ...
    begin() {
        ...
        if (this.directConnect) {
            logString += " with direct connect in " + this.directConnect.timeZone + " timezone with modtd " + this.directConnect.modtd;
        }
    ...
    addClient(client, uuid, secret) {
        ...
        this.log(`Client ${uuid}(${client.data.clientID}) joined`, ADMINS[secret] ? `(admin: ${ADMINS[secret]})` : "");
    }
